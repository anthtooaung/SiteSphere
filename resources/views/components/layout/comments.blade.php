...
<section class="aud-reviews" aria-label="User Reports">
    ...
    <div class="aud-ledger" id="reviewLedger">
        @forelse($comments as $comment)
            ...
            @endphp
            <article
                class="aud-row @if($comment->trashed()) banned-border banned-tooltip @endif"
                id="comment-{{ $comment->id }}"
                data-review-id="comment-{{ $comment->id }}"
                @if($comment->trashed()) data-ban-reason="{{ $comment->getBanReason() ?? 'No reason provided' }}" @endif
                x-data="{
                    actionsOpen: false,
                    editOpen: false,
                                    ...
                                    @else
                                        @if(auth()->user()->role === 'admin')
                                            @if($comment->trashed())
                                                {{-- Revert (Unban) --}}
                                                <div class="mt-1 border-t pt-1 [border-color:color-mix(in_srgb,var(--text-color,#0d1b2a)_10%,transparent)]">
                                                    <form method="POST" action="{{ route('comments.unban', $comment->id) }}">
                                                        @csrf
                                                        <button type="submit"
                                                ...
                                                {{-- Delete Permanently --}}
                                                <div class="mt-1 border-t pt-1 [border-color:color-mix(in_srgb,var(--text-color,#0d1b2a)_10%,transparent)]">
                                                    <form method="POST" action="{{ route('comments.force-delete', $comment->id) }}"
                                                        x-on:submit.prevent="window.sitesphereSwal.confirm({
