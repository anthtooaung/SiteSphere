...
const init = () => {
  ...
  */
  ...
  const openRegistrationFlow = (account, backendOtp = null) => {
    ...
    window.dispatchEvent(new CustomEvent('update-initial', { detail: account.username.charAt(0).toUpperCase() }));
  ...
  const resetRegistrationFlow = () => {
    ...
    clearOtpInputs();
    profileForm?.reset();
    document.getElementById('cropped_avatar') && (document.getElementById('cropped_avatar').value = "");
    
    // dispatch event for Alpine JS to reset
    window.dispatchEvent(new CustomEvent('reset-profile'));
  ...
  const collectProfile = () => ({
    ...
    cropped_avatar: document.getElementById('cropped_avatar')?.value ?? "",
  });
  ...
  const renderConfirmation = () => {
    ...
    if (registrationData.profile.cropped_avatar) {
      confirmAvatar.classList.add("has-image");
      confirmAvatar.innerHTML = `<img src="${registrationData.profile.cropped_avatar}" alt="Profile image preview" />`;
    } else if (registrationData.profileImageDataUrl) {
      confirmAvatar.classList.add("has-image");
    ...
    const rows = [
      ...
      [
        "Profile image",
        (registrationData.profile.cropped_avatar || registrationData.profileImageDataUrl) ? "Selected" : "Not added",
      ],
    ];
  ...
  confirmRegisterBtn?.addEventListener("click", async () => {
    ...
    if (registrationData.profile.cropped_avatar) {
      formData.append("cropped_avatar", registrationData.profile.cropped_avatar);
    } else if (profileImage && profileImage.files[0]) {
...
window.editProfilePage = function(config) {
    return {
